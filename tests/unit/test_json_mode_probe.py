...
def _clear_probe_state():
    """Drop persisted per-model probe answers between tests."""
    from database import Database
    db = Database()
    for key in (_JSON_FORMAT_SETTING_KEY, _JSON_SCHEMA_SETTING_KEY):
...
class TestRuntimeJsonModeFallback:
    ...
    def test_retries_once_and_disables_response_format(self):
        ...
        assert client._get_json_format_supported('test-model') is False
    ...
    def test_unrelated_400_unprobed_endpoint_retry_fails_reraises_original(self):
        ...
        client._record_format_support = MagicMock()
        ...
        assert client._client.chat.completions.create.call_count == 2
        assert client._get_json_format_supported('test-model') is None
        client._record_format_support.assert_not_called()
    ...
    def test_unknown_phrasing_unprobed_endpoint_retry_succeeds_persists(self):
        ...
        assert result.content == "recovered"
        assert client._get_json_format_supported('test-model') is False
        client._record_format_support.assert_called_once()
        assert client._client.chat.completions.create.call_count == 2
        second_kwargs = client._client.chat.completions.create.call_args_list[1][1]
        assert 'response_format' not in second_kwargs
        injected = second_kwargs['messages'][0]['content']
